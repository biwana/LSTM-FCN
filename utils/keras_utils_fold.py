...
from utils.generic_utils_fold import load_dataset_at
from utils.constants import max_seq_len, nb_classes


def train_model(model:Model, dataset_id, method, proto_num, fold, dataset_prefix, nb_iterations=100000, batch_size=128, val_subset=None, cutoff=None, normalize_timeseries=False, opt='Adam', learning_rate=1e-3, early_stop=False, balance_classes=True, run_ver=''):
    X_train1, X_train2, y_train, X_test1, X_test2, y_test, is_timeseries = load_dataset_at(dataset_id, method, proto_num, fold, normalize_timeseries=normalize_timeseries)
    ...
    model_checkpoint1 = ModelCheckpoint("./weights/fold%s_%s_%s_%s_%sloss_weights.h5" % (fold, dataset_prefix, method, str(proto_num), run_ver), verbose=2,
                                       monitor='loss', save_best_only=True, save_weights_only=True)
    model_checkpoint2 = ModelCheckpoint("./weights/fold%s_%s_%s_%s_%sval_acc_weights.h5" % (fold, dataset_prefix, method, str(proto_num), run_ver), verbose=2,
                                       monitor='val_acc', save_best_only=True, save_weights_only=True)

    tensorboard = TensorBoard(log_dir='./logs/fold%s_%s%s_%s_%s' % (fold, run_ver, dataset_prefix, method, str(proto_num)), batch_size=batch_size)
    csv_logger = CSVLogger('./logs/fold%s_%s%s_%s_%s.csv' % (fold, run_ver, dataset_prefix, method, str(proto_num)))
    if early_stop:
...
def evaluate_model(model:Model, dataset_id, method, proto_num,  fold, dataset_prefix, batch_size=128, test_data_subset=None,
    ...
    model.load_weights("./weights/fold%s_%s_%s_%s_%s_weights.h5" % (fold, dataset_prefix, method, str(proto_num), checkpoint_prefix))
