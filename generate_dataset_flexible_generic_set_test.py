...
if __name__ == "__main__":
    if len(sys.argv) < 7:
        print("Error, Syntax: {0} [version] [prototype selection] [classwise/independent] [prototype number] [no_classes] [dimensions]".format(sys.argv[0]))
        exit()
    version = sys.argv[1]
    ...
    # load settings
    full_train_file = os.path.join("data", version + "-train-data.txt")
    full_test_file = os.path.join("data", version + "-test-data.txt")
    full_train_label_file = os.path.join("data", version + "-train-labels.txt")
    full_test_label_file = os.path.join("data", version + "-test-labels.txt")
    # load data
    ...
    full_test_label = np.genfromtxt(full_test_label_file, delimiter=' ')
    ...
    test_data = 2. * (full_test - train_min) / (train_max - train_min) - 1.
    test_labels = full_test_label[:,1]
    #print(np.shape(test_data))
    #print(np.shape(test_labels))

    test_number = np.shape(test_labels)[0]
    #print(test_number)
    seq_length = int(np.shape(test_data)[1] / dimensions)
    #print(seq_length)
    #exit()
